...
function BuildHead(metaObject = {}) {
    ...
    if (metaObject.structuredData) {
        finalString += `<script type="application/ld+json">${JSON.stringify(metaObject.structuredData)}</script>`;
    }
...
function ProcessItem(itemObject) {
    ...
    if (itemObject.script) {
        ...
        finalString = `<script src="${src}"${KeysToString(itemObject)}${extras} ></script>`;
...
function MergeHead(newObject = {}, oldObject = {}) {
    ...
    const mergedObject = deepmerge(oldObject, newObject, { arrayMerge: function concatMerge(destinationArray, sourceArray) {
