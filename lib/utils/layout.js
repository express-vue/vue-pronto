 ...
 */
function BuildLayout(layoutObject, cache) {
    let finishedLayout = {
        start: "",
        end: "",
    };
    ...
    if (layoutObject) {
        ...
        finishedLayout.end = `${layout.template.end}${BuildScript(layoutObject.script, cache)}${layout.body.end}${layout.html.end}`;
    } else {
 ...
 * @returns {String}
 */
function BuildScript(script, cache) {
    ...
    const cached = cache.get(vueString);
    let minified = {};
    if (cached) {
        minified = cached;
    } else {
