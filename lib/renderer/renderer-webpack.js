...
class Renderer {
    ...
    async _loadBundleFilesToMemory(filepaths) {
        ...
        filepaths.forEach(filepath => {
            ...
            vm.mfs.writeFileSync(`${bundlePath.memory.path}.js`, fs.readFileSync(filepath));
        });
    }
    /**
    ...
    getBundleFilePath(filePath) {
        ...
        let memoryParsed = parsedBundlePath;
        if (filePath.includes(".expressvue")) {
            memoryParsed = path.parse(filePath.split(".expressvue")[1]);
        } else {
            memoryParsed = path.parse(filePath.split(this.rootPath)[1]);
        }
        const memoryBase = path.join("/expressvue/bundles", memoryParsed.dir);
    ...
    async _buildConfig(filePath) {
        ...
        try {
            fs.statSync(bundlePath.base).isDirectory();
        } catch (error) {
