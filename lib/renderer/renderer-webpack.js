 ...
 * @prop {import("../models/layout")} template
 */
...
class Renderer {
    ...
    async _buildConfig(filePath) {
        const bundlePath = this.getBundleFilePath(filePath);
        const app = config.appConfig(filePath, this.vue.app);
        const server = config.serverConfig(this.vue.app);
        const client = config.clientConfig(this.vue.client);
