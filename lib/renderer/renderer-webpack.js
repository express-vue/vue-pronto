...
class Renderer {
    ...
    constructor(options = {}) {
        ...
        this.webpackConfig = {
            entry: {
                index: [path.join(this.trueRootPath, "server-entry.js")],
            },
            output: {
                filename: "foo.js",
                path: this.trueRootPath,
    ...
    Compile(filePath) {
        return new Promise((resolve, reject) => {
            ...
            fs.writeFileSync(serverEntryPath, webpackImport, "utf-8");
    ...
    MakeBundle(compiled, filePath) {
        return new Promise((resolve, reject) => {
            const compiler = Webpack(compiled.webpackConfig);
            ...
            function getFileName(serverConfig, projectName) {
                return serverConfig.output.filename.replace(
                    "[name]",
                    projectName,
                );
            }
            const outputPath = path.join(
                fileOutput.path,
                getFileName(compiled.webpackConfig, "projectName"),
            );
                    ...
                    .then(compiled => {
                        this.MakeBundle(compiled, filePath)
                            .then(bundle => {
                                ...
                                const thing = createBundleRenderer(
                                    bundle.bundle,
                                );
                ...
                .then(filePath => {
                    this.MakeVueClass(filePath, data)
                        .then(vueClass => {
                            ...
                            const vueStream = this.renderer.renderToStream(
                                vueClass.vue,
                            );
                            let htmlStream;
                            const layout = Utils.BuildLayout(context);
