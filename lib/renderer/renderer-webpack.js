...
class Renderer {
     ...
     */
    async Bootstrap(forceBuild) {
        if (forceBuild || (!process.env.VUE_DEV || (process.env.VUE_DEV && process.env.VUE_DEV === "false"))) {
            const vm = this;
            ...
            for (let index = 0; index < filepaths.length; index++) {
                ...
                const bundleClientFile = `${this.bundlePath}/${path.parse(filepath).base}.client.js`;
                if (forceBuild || !fs.existsSync(bundleClientFile)) {
                    try {
                    ...
                    console.info(`Using Compiled file in ${this.bundlePath}`);
                }
            }
            try {
                ...
                stats.forEach(async stat => {
                    //@ts-ignore
                    const writeBundleFiles = stat.stats.map(item => {
                        return this._writeBundleFile(item.compilation, item.compilation.outputOptions.filename);
                    });
                    await Promise.all(writeBundleFiles);
                });
    ...
    async _writeBundleFile(element, filename) {
        if (filename.includes(".client.js")) {
            try {
                ...
                return await mz.writeFile(`${this.bundlePath}/${filename}`, element.assets[filename]._value);
            }
        }
    }
    /**
