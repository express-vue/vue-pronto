...
class Renderer {
    ...
    Compile(filePath) {
        return new Promise((resolve, reject) => {
            ...
            this.mfs.mkdirpSync("/vuepronto");
            this.mfs.writeFileSync("/vuepronto/server-entry.js", webpackImport, "utf-8");
            this.webpackConfig.entry.index = ["/vuepronto/server-entry.js"];
            const compiled = {
    ...
    MakeBundle(compiledObject, filePath) {
        return new Promise((resolve, reject) => {
            const webpackCompiler = Webpack(compiledObject.webpackConfig);
            const fileOutput = compiledObject.webpackConfig.output;
            webpackCompiler.outputFileSystem = this.mfs;
            webpackCompiler.watch({}, (error, stats) => {
                if (error) { reject(error); }
                try {
                    const bundle = this.mfs.readFileSync(path.join(fileOutput.path, fileOutput.filename), "utf-8");
                    resolve({
                        bundle: bundle,
                        compiled: compiledObject.compiled,
                    });
                } catch (error) {
                    reject(error);
                }
