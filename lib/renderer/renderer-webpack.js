...
class Renderer {
    ...
    async _makeVueClass(filePath, context) {
        ...
        try {
            const bundlePath = this.getBundleFilePath(filePath);
            context.script = path.sep === '/' ? bundlePath.memory.filename.client : bundlePath.memory.filename.client.replace(/^C:/, ''); // strip C: for windows systems
            ...
            context.script = path.sep === '/' ? bundle.clientBundlePath : bundle.clientBundlePath.replace(/^C:/, ''); // strip C: for windows systems
            const layout = buildLayoutWebpack(context);
    ...
    getBundleFile(bundleFileName) {
        const fullFileName = path.sep === '/' ? bundleFileName : path.join('C:\\', bundleFileName); // add C: for memory file system for windows machines
