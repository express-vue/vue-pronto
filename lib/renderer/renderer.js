...
class Renderer {
    ...
    FixPlugins(plugins) {
        /** @type {Plugin[]} */
        let pluginArray = Object.assign([], this.plugins, plugins);
        let finalArray = [];

        if (pluginArray.length > 0) {
            for (let index = 0; index < pluginArray.length; index++) {
                const element = pluginArray[index];
                finalArray.push(new Plugin(element));
            }
        }
                ...
                .then(filePath => {
                    this.MakeVueClass(filePath, data)
                        .then(vueClass => {
                            ...
                            const layout = Utils.BuildLayout(context, vueOptions.plugins, this.nodeModulesPath);
                ...
                .then(filePath => {
                    this.MakeVueClass(filePath, data)
                        .then(vueClass => {
                            ...
                            const template = Object.assign({}, this.template, vueOptions.template);
                            const plugins = this.FixPlugins(vueOptions.plugins);
                            //Init Renderer
                            const context = {
                                head: headString,
                                template: template,
                            ...
                            let htmlStream;
                            const layout = Utils.BuildLayout(context, plugins, this.nodeModulesPath);
